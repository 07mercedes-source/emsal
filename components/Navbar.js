    ...
    <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: 12, background: "#0b1220", color: "#fff", position: "sticky", top: 0, zIndex: 1100 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center", cursor: "pointer" }} onClick={() => router.push("/")}>
          <img src="/logo.png" alt="EMSAL" style={{ height: 72, marginRight: 8, background: "#fff", borderRadius: 8, padding: 6 }} />
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{ color: "#9fb2ff", fontWeight: 600 }}>{user ? `${user.name} • ${user.role || "Personel"}` : ""}</div>
        ...
        {user ? (
          <button style={{ ...actionBtn, background: "#ef4444", color: "#fff" }} onClick={() => { logout(); router.push("/login"); }}>{t("logout")}</button>
        ) : (

...
export default function Restaurant2(){
  const { restaurant2, setRestaurant2 } = useRestaurantData();
  const [transactions, setTransactions] = useState([]);

  useEffect(()=> {
    const example = [
      { id: 1, type: "Gelir", desc: "Satış", amount: 3200, date: "2025-10-01" },
      { id: 2, type: "Gider", desc: "Et alımı", amount: 1200, date: "2025-10-02" },
    ];
    if((restaurant2||[]).length === 0){
      setTransactions(example);
      setRestaurant2(example);
    } else {
      setTransactions(restaurant2);
    }
  },[]);

  useEffect(()=> setRestaurant2(transactions), [transactions]);

  const add = (type) => setTransactions([...transactions, { id: Date.now(), type, desc:"", amount:0, date: new Date().toISOString().slice(0,10) } ]);
  ...
  const exportExcel = async () => {
    ...
    XLSX.writeFile(wb, "restaurant2_rapor.xlsx");
  };
  ...
  const exportPDF = async () => {
    ...
    const doc = new jsPDF();
    doc.text("Restaurant 2 Rapor",14,15);
    doc.autoTable({ startY:20, head:[["ID","Tür","Açıklama","Tutar","Tarih"]], body: transactions.map(t=>[t.id, t.type, t.desc, t.amount, t.date]) });
    doc.save("restaurant2_rapor.pdf");
  };
